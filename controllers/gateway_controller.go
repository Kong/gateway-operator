...
func (r *GatewayReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	// TODO: status updates, AND we need to add Gateway conformance tests

	debug(log, "looking for dataplanes associated with gateway resource", gateway)
	dataplane := new(operatorv1alpha1.DataPlane)
	dataplaneNSN := types.NamespacedName{Namespace: req.Namespace, Name: "dataplane-" + req.Name} // TODO: generated
	...
	controlplaneNSN := types.NamespacedName{Namespace: req.Namespace, Name: "controlplane-" + req.Name} // TODO: generated
...
func (r *GatewayReconciler) CreateControlPlane(ctx context.Context, gateway *gatewayv1alpha2.Gateway, dataplaneName string) error {
	gatewayClassName := gatewayv1alpha2.ObjectName("kong") // FIXME: dynamic

	controlplane := &operatorv1alpha1.ControlPlane{
		ObjectMeta: metav1.ObjectMeta{
			Namespace: gateway.Namespace,
			Name:      "controlplane-" + gateway.Name, // FIXME
		},
		Spec: operatorv1alpha1.ControlPlaneSpec{
			GatewayClass: &gatewayClassName,
			DataPlane:    kong.String(dataplaneName), // TODO: use ctrlrt's version of this instead
		},
	}
...
func (r *GatewayReconciler) ensureGatewayMarkedReady(ctx context.Context, gateway *gatewayv1alpha2.Gateway) error {
	...
	if !ready {
		...
		if len(svc.Status.LoadBalancer.Ingress) > 0 {
			gatewayIPs = append(gatewayIPs, svc.Status.LoadBalancer.Ingress[0].IP) // TODO: handle hostnames, e.t.c.
		}
